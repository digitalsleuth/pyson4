...
"""
This is a Python 3 script for converting the Firefox lz4 type file header to readable JSON.
The Firefox jsonlz4 'file-type' is unique in that the header (mozLz40\x00) is not a standard LZ4 frame type.
...
__date__ = '30 Nov 2019'
__version__ = '1.0'
__description__ = 'Firefox JSON LZ4 Parsing Tool'
...
if __name__ == "__main__":
    arg_parse = ArgumentParser(description="Firefox JSONLZ4 parsing tool", epilog="This tool will only work with the Mozilla Firefox lz4 format with Firefox.\nThis includes the jsonlz4 and mozlz4 file extensions.\nFor other LZ4 utilities, try the python-lz4 library", formatter_class=RawTextHelpFormatter)
    ...
    arg_parse.add_argument("--verbose", action="store_true", help="Verbose - prints output path and size.\nBetter to not use if iterating through files")
    ...
    try:
        if args.d:
            ...
            try:
                ...
                if args.verbose:
                    print("Output file '%s' created. %s bytes output." % (args.output_file, file_size))
            finally:
                output_file.close()                      
        elif args.c:
            ...
            print("Please choose either the -d (decompress) or -c (compress) option to proceed.")
            raise SystemExit(0)
